...
func getHighestToken(mapTokenizer map[string]interface{}) (int64, error) {
	// get all unique minted tokens
	dataOrdering, tfOK := mapTokenizer["ordering"]
	if !tfOK {
		return -1, errors.New("list ordering does not exist")
	}

	// get ordering list's last element
	alMerges, tfOK := dataOrdering.([]interface{})
	if !tfOK {
		return -1, errors.New("cannot convert list to interface")
	}
	aaiPairs := alMerges[len(alMerges)-1]

	// convert to list again
	aiPairs := aaiPairs.([]interface{})

	// generate lookup key
	sLookupKey := keyToString([2]int64{int64(aiPairs[0].(float64)), int64(aiPairs[1].(float64))})

	// lookup merges dictionary
	...
	if !tfOK {
		return -1, errors.New("cannot convert map to interface")
	}

	// lookup key
	iMintedToken, tfOK := mapMerges[sLookupKey]
	if !tfOK {
		return -1, errors.New("lookup doesn't exist")
	}

	// convert to int64
	lMintedToken, tfOK := iMintedToken.(float64)
	if !tfOK {
		return -1, errors.New("incorrect data type")
	}

	return int64(lMintedToken), nil
}
